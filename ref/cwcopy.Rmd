...
scatter3D(x, y, z, pch = 19, cex = 1,colvar = NULL, col="red", 
          ...
          surf = list(x = rangex, y = rangey, z = rangez,  
                      facets = TRUE, fit = fitpoints, col=ramp.col (col = c("dodgerblue3","seagreen2"), n = 300, alpha=0.9), border="black"), main = "Count linear approximation")
```
...
while(tf==0&n<=25){#maximum number of loop is 25
  ...
  z <- etaval + (Ylin-mu)*1/mu #eta minus normalized value to adjust variable
  w <- mu#update a weight
  fitlin <- lm(z~read$attention+read$fluency,weights = w)#regress y on the components of data with weight w
  betapoi <- as.vector(fitlin$coeff)
  new_D <- D(invlog(Xpoi%*%betapoi))
  control <- abs(new_D-old_D)/(abs(new_D)+0.1) #plus 0.1 so that it wont be divided by zero
  if(control<1e-8)
    tx <- 1
  old_D <- new_D
  n<-n+1
...
```{r}
fit1 <- glm(Ypoi~read$attention+read$fluency,data=read,family=gaussian(link='log'),start = c(0,0,0))
plot(fit1)



fit3 <- lm(Ypoi~read$attention+read$fluency+read$attention*read$fluency)
plot(fit3)
plot(fitlin)

fit4 <- lm(Ypoi~read$attention+read$fluency+read$yr*read$attention+read$fluency*read$yr)
plot(fit4) 
read
summary(fit4)
```
...
fit6 <- glm(Ylin~read$attention+read$fluency+read$yr,family = poisson(link="log"))
plot(fit6) 
summary(fit6)


fit7 <- glm(Ylin~read$attention+read$fluency+read$yr,family = gaussian(link="identity"))
plot(fit7) 
summary(fit7)


Yplusone <- Ylin+1 
fit8 <- glm(Yplusone~read$attention+read$fluency+read$yr,family = Gamma(link="log"),start=c(1,1,1,1))
plot(fit8) 
summary(fit8)
#note need to minus one for the value we get
```
...
Flinlog<-((RSS0-RSSlinlog)/RSSlinlog)*((length(Ylinlog)-3)/3-1)
Flinlog

#get the critical region for f distribution (here it is the value for 0.01)
fcrit <- qf(0.01,df1=3-1,df2=length(Ylinlog)-3,lower.tail = FALSE)
fcrit
```
