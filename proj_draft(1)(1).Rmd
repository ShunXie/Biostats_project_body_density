...
dim(bd_df)
bd_df <- bd_df%>%
  dplyr::select(bodyfat_brozek, age:wrist)%>%
...
cv_object <- cv.glmnet(as.matrix(bd_df[,-1]), bd_df$bodyfat_brozek, lambda = lambda_seq, 
nfolds = 5)
cv_object
...
```{r}
set.seed(100)
fit_bestcv <- glmnet(X.train, Y.train, lambda = best_lambda)
coef(fit_bestcv)
sqrt(cv_object$cvm[cv_object$lambda == cv_object$lambda.min])
...
Model fitted with LASSO regression has the lowest rmse in four models. Therefore we would recommend lasso model
